...
pos <- read.csv("metabolomics_pos.csv")
...
nonmiss.hp <- posdat %>% group_by(Match_Caseset.x) %>% filter(!anyNA(HPPOS)) %>% ungroup() # 400

# Condordant case-control pairs
concordant11 <- nonmiss.hp %>% group_by(Match_Caseset.x) %>% filter(sum(HPPOS) == 2) %>% ungroup() # 210/2 = 105
concordant00 <- nonmiss.hp %>% group_by(Match_Caseset.x) %>% filter(sum(HPPOS) == 0) %>% ungroup() # 22/2 = 11

# Discordant case-control pairs: all
discordant   <- nonmiss.hp %>% group_by(Match_Caseset.x) %>% filter(sum(HPPOS) == 1) %>% ungroup() # 168/2 = 84
table(discordant$HPPOS, discordant$Cncr_Caco_Stom.x) # 67 and 17 of each discordance type
...
anymiss.hp <- posdat %>% group_by(Match_Caseset.x) %>% filter(is.na(sum(HPPOS))) %>% ungroup() # 474/2 = 237

# Identify CC pairs with both missings
allmiss.hp <- posdat %>% group_by(Match_Caseset.x) %>% filter(sum(is.na(HPPOS))==2) %>% ungroup() # 460/2 = 230

# Identify CC pairs with an HPPOS == 1 and a missing
miss.hp1 <- miss.hp %>% group_by(Match_Caseset.x) %>% filter(sum(HPPOS, na.rm = TRUE) == 1) %>% ungroup() # 10/2 = 5
...
# Neg mode ----
# Mostly same code as above but repeated for better clarity
# Join participant data to metabolomics data
library(tidyverse)
negdat <- right_join(dat, neg, by = "Idepic")

negdat %>% group_by(Match_Caseset.x) %>% filter(any(HPPOS == 1)) %>% n()


# Keep CC pairs with no HPPOS missings
nonmiss.hp <- negdat %>% group_by(Match_Caseset.x) %>% filter(!anyNA(HPPOS)) %>% ungroup() # 400
...
# Identify CC pairs with at least one missing
anymiss.hp <- negdat %>% group_by(Match_Caseset.x) %>% filter(is.na(sum(HPPOS))) %>% ungroup() # 474/2 = 237

# Identify CC pairs with both missings
allmiss.hp <- negdat %>% group_by(Match_Caseset.x) %>% filter(sum(is.na(HPPOS))==2) %>% ungroup() # 460/2 = 230
